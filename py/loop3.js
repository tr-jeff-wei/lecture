...
function convertForLoop(jsLoop) {
  if (jsLoop.indexOf("for") != -1) {
    return jsLoop
      .replace("let i=", "i in range(")
      .replace(" ; i<", ",")
      .replace(" ; i=i+", ",")
      .replace(" ; j<=", ",")
      .replace(" ; j=j+", ",")
  ...
  if (jsLoop.indexOf("}") != -1) {
    return jsLoop.replace("}", "###");
  }
  if (jsLoop.indexOf("if") != -1) {
    return jsLoop.replace("){", ":").replace('(','');
  }
  return jsLoop;
}
...
function showInfo() {
  ...
  if (frameCount - info_timer < 30) text(info, 70, 500);
  pop();
}
...
function setCode() {
  ...
  codeLines.push(``);
  let maxn = rN(5) + 10;
  codeLines.push(`for( let i=0 ; i<${maxn} ; i=i+${1} ){`);
  let rk = rN(3) ;
  if( rk==0 ){
      codeLines.push(`    if( i > ${ Math.floor(maxn/2)} ){`);
  }else if( rk==1){
      codeLines.push(`    if( i < ${ Math.floor(maxn/2)} ){`);
  }else{
      codeLines.push(`    if( i % ${random([2,3,5,7]) } == 0 ){`);
  }
  
  codeLines.push(`        s = s + '🛠️' ;`);
  codeLines.push(`    }`);
  codeLines.push(`}`);
  codeLines.push(``);
...
function draw() {
  ...
  while (true) {
    ...
    text(user_input[uii], spos, last_y + tline * 30);
    spos += textWidth(user_input[uii]);
    }
    uii++;
    if (uii >= user_input.length) {
      break;
    } else if (uii % 10 == 0 && uii > 0) {
      spos = start_x + 85;
      tline++;
    }
...
function drawHighlightedLine(line, x, y) {
  line = convertForLoop(line)
  line = line.replace(";","").replace('let ','')
