...
function setCode_function(used, argsn) {
  ...
  let args = Math.floor(Math.random() * 4);
  ...
  for (let i = 0; i < args; i++) {
    ...
    argsStr += alphabet[i];
    argsNums += Math.floor(Math.random() * 5) + 1;
  }
  argsn.push(argsNums);
  ...
  if (args > 0) {
    codeLines.push(`    return ${alphabet[Math.floor(Math.random() * args)]} ;`);
  } else {
    codeLines.push(`    return ${Math.floor(Math.random() * 10)} ;`);
  }
...
function setCode() {
  ...
  codeLines.push(``);
  codeLines.push(`for( let i=0 ; i<${Math.floor(Math.random() * 5) + 2} ; i=i+${1 + Math.floor(Math.random() * 2)} ){`);
  codeLines.push(`    s = s + '🚀' ;`);
  codeLines.push(`}`);
  codeLines.push(``);
  codeLines.push(`for( let i=0 ; i<${Math.floor(Math.random() * 5)+ 2} ; i=i+${1 + Math.floor(Math.random() * 2)} ){`);
...
function drawHighlightedLine(line, x, y) {
  line = line.replace('for( let ','for ')
    .replace('=0',' in range(0,')
    .replace('; i<','')
  .replace('; i=i+',',')
  .replace('{',' :')
  .replace('}','')
